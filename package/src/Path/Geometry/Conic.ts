...
const isCloseEnough = (curve: ConicCurve, tolerance: number = 0.01): boolean => {
    let dx = curve.p2[0] - curve.p0[0];
    let dy = curve.p2[1] - curve.p0[1];
    let d = Math.abs(dx * (curve.p0[1] - curve.p1[1]) - dy * (curve.p0[0] - curve.p1[0]));
    
    return d < tolerance;
};

// Function to convert a conic curve into a quadratic Bezier
const toQuadratic = (curve: ConicCurve) => new QuadraticPathComponent(curve.p0, curve.p1, curve.p2);

// Function to recursively chop a conic curve into quadratic Beziers
const chopCurve = (curve: ConicCurve, curves: QuadraticPathComponent[] = []) => {
    if (isCloseEnough(curve)) {
        curves.push(toQuadratic(curve));
    } else {
        let [left, right] = splitCurve(curve);
...
export const ConvertConicToQuads = (p0: Point, p1: Point, p2: Point, w: number) => {
